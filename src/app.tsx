...
import { generateImageFromHtml, generateRandomFilename, downloadImage, copyImageToClipboard } from './components/HtmlToImageGenerator';
...
export function App({ note }: ShareImageProps): JSXInternal.Element {
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      container.style.width = isMobile ? '95vw' : '600px'; // 固定宽度
      ...
      setErrorMessage(`生成图片时发生错误: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col justify-start items-center p-2 md:p-5 bg-background overflow-hidden">
      {/* Template selector */}
      ...
      {isGenerating && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-4 md:p-5 rounded-lg shadow-lg flex flex-col items-center">
            ...
            <p className="text-gray-800 text-sm md:text-base">正在生成图片...</p>
          </div>
        </div>
      )}
    </div>
